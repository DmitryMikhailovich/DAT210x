...
for i, phone_num in enumerate(in_numbers):
    ...
    print("\n\nExamining person: ", i, ' with number ', phone_num)
    # 
    # TODO: Create a slice called user1 that filters to only include dataset records where the
    # "In" feature (user phone number) is equal to the first number on your unique list above
    #
    user1 = df[df['In'] == phone_num]
    
    
    #
    # TODO: Alter your slice so that it includes only Weekday (Mon-Fri) values.
    #
    user1 = user1[(user1.CallDate.dt.weekday != 5) & (user1.CallDate.dt.weekday != 6)]
    ...
    #
    ax = plt.subplot(111)
    ax.scatter(user1.TowerLon, user1.TowerLat, c='b', alpha='.3')
